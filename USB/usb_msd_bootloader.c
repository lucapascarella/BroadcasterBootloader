...
int main(void) {
    ...
    if (RCONbits.POR || RCONbits.BOR) {
        ...
        // RCONbits.SWR == 1 // 1 = Software Reset was executed
        // persistent storage valid SoftReset or MCLR is occurred
        if (ALRMDATEbits.WDAY01 == 5 && ALRMDATEbits.DAY01 == 5 && ALRMDATEbits.DAY10 == 2) {
            flash = TRUE;
            ALRMDATEbits.WDAY01 = 0;
            ALRMDATEbits.DAY01 = 1;
            ALRMDATEbits.DAY10 = 0;
        }
    }

    // Check if, software upgarde or Switch trigger or invalid app ask to upgare the firmware
    if (!flash && !CheckTrigger() && ValidAppPresent()) {
        // This means the switch is not pressed. Jump directly to the application
        JumpToApp();
    }
